...
test('with no game yet, it still reads as a post', () => {
  ...
  assert.ok(msg.startsWith('Save this to your home screen'));
});
...
// The post is a recipe, and the order is the whole of it: save it, run the
// trick, then the payoff. Installing is what makes the illusion exist, but as a
// bare instruction it reads as homework — it only works as step one of a prank
// the reader has already decided to pull.
test('every post reads as a prank in order', () => {
  for (const msg of [xMessage(), xMessage({ score: 3, total: 24 })]) {
    assert.ok(msg.startsWith('Save this to your home screen'), 'starts with the save');
    assert.match(msg, /hand them your phone/i, 'sets the trick up');
    assert.match(msg, /hold down the Search bar/i, 'says what triggers it');
    assert.match(msg, /their own phone/i, 'lands the payoff');
    assert.equal(msg.match(/hold down the Search bar/gi).length, 1, 'said once');
  }
});

// The real joke is not the basketballs — it is someone holding down Search on a
// real iPhone afterwards. That beat has to come last or there is nothing to
// laugh at.
test('the payoff comes after the trick, never before', () => {
  const msg = xMessage({ score: 17, total: 24 });
  assert.ok(msg.indexOf('their own phone') > msg.indexOf('Search bar'), 'payoff last');
  assert.ok(msg.indexOf('hand them your phone') > msg.indexOf('home screen'), 'save first');
});
...
test('the score never comes before the ask', () => {
  const msg = xMessage({ score: 17, total: 24 });
  assert.ok(msg.indexOf('I got') > msg.indexOf('home screen'), 'save first');
  assert.ok(msg.indexOf('I got') > msg.indexOf('their own phone'), 'prank before the brag');
});
